...
private[cluster] final class ClusterCoreDaemon(publisher: ActorRef) extends Actor with ActorLogging {
  ...
  def joinSeedNodes(seedNodes: immutable.IndexedSeq[Address]): Unit = {
    ...
    if (seedNodes.isEmpty || (seedNodes.size == 1 && seedNodes.head == selfAddress))
      self ! JoinTo(selfAddress)
    else if (seedNodes.head == selfAddress)
      seedNodeProcess = Some(context.actorOf(Props(new FirstSeedNodeProcess(seedNodes)).
        withDispatcher(UseDispatcher), name = "firstSeedNodeProcess"))
    else
      seedNodeProcess = Some(context.actorOf(Props(new JoinSeedNodeProcess(seedNodes)).
  ...
  def join(address: Address): Unit = {
    if (address.protocol != selfAddress.protocol)
      log.info("Trying to join member with wrong protocol, but was ignored, expected [{}] but was [{}]",
        selfAddress.protocol, address.protocol)
    else if (address.system != selfAddress.system)
      log.info("Trying to join member with wrong ActorSystem name, but was ignored, expected [{}] but was [{}]",
...
private[cluster] final class FirstSeedNodeProcess(seedNodes: immutable.IndexedSeq[Address]) extends Actor with ActorLogging {
  ...
  override def preStart(): Unit = {
    ...
    // watch the other seed nodes to get quick feedback of unstarted seed node
    seedNodes.collect {
  ...
  def done: Actor.Receive = {
    ...
    case InitJoinNack(_) ⇒ // not interesting any more
    case Terminated(_)   ⇒ // not interesting any more
